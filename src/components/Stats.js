...
const Stats = (props) => {
  ...
  const { data2, error2 } = useSWR(swrUrl2, (url2) =>
    axios(url2).then((r2) => r2.data)
  );
  ...
  const paymentThreshold =
    data?.pool.paymentProcessing.minimumPayment + " " + data?.pool.coin.type;
  ...
  const poolHashrates = data2?.stats.map((stats) => {
    return [
      ...poolHashrates,
      {
        value: stats.poolHashrate / 1000000000,
        ...
        {!data ? (
          ...
          <Grid container spacing={2} direction="row">
            ...
            <CardChart
              ...
              CardLateststat={hashformat(poolHashRate, 2, "H/s")}
            />
            <CardChart
              data={connectedMiners}
              CardSubtitle="Miners"
              CardLateststat={miners}
            />
            {InfoCard()}
            <CardChart
              data={networkHashRates}
              CardSubtitle="Network Hashrate"
              CardLateststat={hashformat(networkHashrate, 2, "H/s")}
            />
            <CardChart
              data={networkDifficulty}
              CardSubtitle="Network Difficulty"
              CardLateststat={networkDifficultyValue}
            />
          </Grid>
        )}
      </div>
